...
model_path = os.path.join(os.path.abspath("."), "weights", "yolov8n.pt")
...
while True:
    ...
    for i, xyxy in enumerate(xyxy_list):
        # 针对 "杯子" 类别（COCO类别ID为41）
        if class_id_list[i] != 41:  
            continue
        ...
        cam_point3d = camera.depth_pixel2cam_point3d(cx, cy, depth_value)
