...
int configMultiDeviceSync() try {
    ...
    for(auto device: configDevList) {
        ...
        if(IsGemini330Series(pid)) {
            // Gemini 330 Series cannot be restarted because the parameters will not be saved after reboot.
            continue;
        }
    ...
    // Wait for the device to restart
    {
        ...
        std::cout << "All device update config (or reboot) complete." << std::endl;
    }
